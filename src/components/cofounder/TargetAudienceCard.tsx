...
const TargetAudienceCard = ({ data }) => (
  <SectionCard title={data.title} summary={data.summary} cta={data.cta}>
    ...
    {data.personas.map((p, i) => (
      <div key={i} className="p-6 rounded-lg grid md:grid-cols-3 gap-6 bg-secondary">
        <div>
        ...
        <div className="md:col-span-2 space-y-4">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {p.pain_points.map((tag, j) => <span key={j} className="text-xs font-medium px-2.5 py-1 rounded-full bg-primary text-primary-foreground">{tag}</span>)}
            </div>
          </div>
          <div>
